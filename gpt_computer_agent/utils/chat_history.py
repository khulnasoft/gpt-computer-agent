    ...
    def __dict__(self):
        ...
        if self.auto_delete is not None:
            ...
            if current_time - self.timestamp > self.auto_delete:
                return {
                    "type": "human",
                    "content": "This content deleted.",
                    "timestamp": self.timestamp,
                    "auto_delete": self.auto_delete,
                    "that_was_empty": self.that_was_empty,
                }
...
class ChatHistory:
    def __init__(self):
        ...
        self.db = KOT(
            f"chat_history_{self.chat_id}", folder=artifacts_dir, enable_hashing=True
        )
            ...
            if message["type"] == "human":
                the_chat.append(
                    Human(
                        ...
                        the_time=message["timestamp"],
                        auto_delete=message["auto_delete"],
                    )
                )
            elif message["type"] == "assistant":
                the_chat.append(
                    Assistant(content=message["content"], the_time=message["timestamp"])
                )
            elif message["type"] == "system":
                the_chat.append(
                    System(content=message["content"], the_time=message["timestamp"])
                )
