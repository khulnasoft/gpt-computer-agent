...
This is websearch
    ...
    manager = SyncInvocationManager(
        command="npx", args=["-y", "@mzxrai/mcp-webresearch"]
    )
    manager.start()
    tool_manager = MCPToolManager(manager)
    tools = tool_manager.load_tools()
...
def custom_mcp_servers():
    ...
    if custom_mcp_severs_ == previous_mcp_servers:
        ...
        previous_mcp_server_names = [
            mcp_server["name"] for mcp_server in loaded_mcp_servers
        ]
        # Get the names of the mcp servers that are in the custom_mcp_servers_ list
        custom_mcp_server_names = [
            mcp_server["name"] for mcp_server in custom_mcp_severs_
        ]
        # Get the names of the mcp servers that are not loaded
        mcp_server_names_to_load = list(
            set(custom_mcp_server_names) - set(previous_mcp_server_names)
        )
        ...
        for mcp_server in custom_mcp_severs_:
            if mcp_server["name"] in mcp_server_names_to_load:
                manager = SyncInvocationManager(
                    command=mcp_server["command"], args=mcp_server["args"]
                )
                manager.start()
                tool_manager = MCPToolManager(manager)
                tools = tool_manager.load_tools()
...
def remove_custom_mcp_server(name: str):
    global custom_mcp_severs_
    custom_mcp_severs_ = [
        mcp_server for mcp_server in custom_mcp_severs_ if mcp_server["name"] != name
    ]
