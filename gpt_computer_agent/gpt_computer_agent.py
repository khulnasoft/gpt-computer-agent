    ...
    def run(self):
        while True:
            ...
            if self.the_input_text:
                last_text = self.commited_text[-1] if len(
                    self.commited_text) > 0 else ""
...
class CustomTextEdit(QTextEdit):
    ...
    def keyPressEvent(self, event):
        ...
        super(CustomTextEdit, self).keyPressEvent(
            event)  # Process other key events normally
        ...
        while True:
            ...
            if self.the_input_text and (self.prev == None or self.prev != self.the_input_text):
                ...
                for i in range(len(self.title_bar_text)):
                    self.text_to_set_title_bar.emit(
                        self.title_bar_text[:i + 1])
                    self.msleep(10)
...
class DrawingWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if not self.main_.state == "thinking":
            # Green color with 2px thickness
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # In order to initialize the painting, it will be overwritten by the settings
    ...
    def start_border_animation(self, status):
        ...
        if status:
            ...
            self.title_bar.setStyleSheet(
                f"background-color: #2E2E2E; color: white; border-style: solid; border-radius: 15px; border-width: 0px; color: #fff;")
    ...
    def general_styling(self):
        ...
        self.setStyleSheet(
            "border-radius: 10px; background-color: rgba(45, 45, 45, 250);")
    ...
    def dark_mode(self):
        ...
        p.setColor(self.backgroundRole(), QColor("#171717"))
        self.setPalette(p)
        self.input_box.setStyleSheet(
            self.input_box_style+"background-color: #2E2E2E; color: white;")

        self.send_button.setStyleSheet(
            self.send_button_style+"background-color: #2E2E2E; color: white;")
        self.screenshot_button.setStyleSheet(
            self.screenshot_button_style+"background-color: #2E2E2E; color: white;")
    ...
    def light_mode(self):
        ...
        self.send_button.setStyleSheet(
            self.send_button_style+"background-color: #FFFFFF; color: black; ")
        self.screenshot_button.setStyleSheet(
            self.screenshot_button_style+"background-color: #FFFFFF; color: black; ")
        self.settingsButton.setStyleSheet(
    ...
    def initUI(self):
        ...
        self.shortcut_screenshot.activated.connect(
            lambda: self.button_handler.toggle_recording(
                take_system_audio=True)
        )
        ...
        def input_box_send_screenshot():
            if input_box.toPlainText() != "":
                self.button_handler.input_text_screenshot(
